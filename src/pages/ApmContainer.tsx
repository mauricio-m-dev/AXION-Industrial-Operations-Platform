...
import { ApmHealthTab } from "./ApmHealthTab";
import { ApmWebhooksTab } from "./ApmWebhooksTab";
import { useLanguage } from "../contexts/LanguageContext";

export function ApmContainer() {
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-2">
          <h2 className="text-2xl font-bold text-zinc-900 dark:text-zinc-100 tracking-tight flex items-center gap-2">
            <Activity className="text-[#DC2626]" size={28} />
            {t("admin.nav.health") || "APM & Monitoramento"}
          </h2>
          <p className="text-zinc-500 dark:text-zinc-400 font-medium text-sm">
            Gerencie a saúde do sistema e as configurações de roteamento de alertas operacionais.
          </p>
        </div>
      ...
      {/* Render Active Tab */}
      <div className="mt-2">
        {activeTab === "health" && <ApmHealthTab />}
        {activeTab === "webhooks" && <ApmWebhooksTab />}
      </div>
    </div>
  );
}
